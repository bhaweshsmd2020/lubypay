...
<?php $__env->startSection('page_content'); ?>
    ...
    <div class="box">
        <div class="box-body">
            <form class="form-horizontal" action="<?php echo e(url('admin/transactions')); ?>" method="GET" id='transaction_form'>
                                ...
                                <?php $__currentLoopData = $transactions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index=>$transaction): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php
                                        ...
                                        switch ($transaction->transaction_type_id)
                                        {
                                            ...
                                            case 39:
                                            case Cable:
                                                if (!empty($transaction->user) && !empty($transaction->user->first_name))
                                                {
                                    ...
                                    <?php if($transaction->transaction_type_id != '2' || ($transaction->transaction_type_id == '2' && $transaction->pay_type != '2')): ?>
                                        <tr>
                                            <td><?php echo e(++$index); ?></td>
                                            <td><?php echo e($transaction->uuid); ?></td>
                                            <td><?php echo e(Carbon\Carbon::parse($local_time)->format('d-M-Y h:i A')); ?></td>
                                            <td><a href="<?php echo e(url('admin/users/edit/' . $transaction->user_id)); ?>"><?php echo e(!empty($sender) ? $sender : '-'); ?></a></td>
                                            <td><?php echo e($transaction->currency->type != 'fiat' ? $transaction->subtotal : formatNumber($transaction->subtotal)); ?></td>
                                            <td><?php echo e(($transaction->charge_percentage == 0) && ($transaction->charge_fixed == 0) ? '-' : ($transaction->currency->type != 'fiat' ? $transaction->charge_fixed : formatNumber($transaction->charge_percentage + $transaction->charge_fixed))); ?></td>
                                            <td><?php echo e($transaction->total); ?></td>
                                            <td><?php echo e($transaction->currency->code); ?></td>
                                            <td>
                                                <?php if($transaction->is_card == '1'): ?>
                                                    Virtual Card
                                                <?php else: ?>
                                                    <?php echo e(($transaction->transaction_type->name == "Withdrawal") ? "Payout" : str_replace('_', ' ', $transaction->transaction_type->name)); ?>

                                                <?php endif; ?>
                                            </td>
                                            <td><a href="<?php echo e(url('admin/users/edit/' . $transaction->end_user_id)); ?>"><?php echo e(!empty($receiver) ? $receiver : '-'); ?></a></td>
                                            <td>
                                            ...
                                            <td>
                                                <?php if(Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_transaction')): ?>
                                                    <a href="<?php echo e(url('admin/transactions/edit/' . $transaction->id)); ?>" class="label label-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;
                                                <?php endif; ?>
                                            </td>
                                        </tr>
                                    <?php endif; ?>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('admin.layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/lubypay/public_html/accounts/resources/views/admin/transactions/index.blade.php ENDPATH**/ ?>