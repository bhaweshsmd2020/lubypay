    ...
    <li class="<?= isset($menu) && ($menu == 'dashboard') ? 'active' : '' ?>">
     ...
     <?php if(Common::has_permission(auth()->id(),'manage_merchant')): ?>
                    <li class="<?= isset($menu) && ($menu == 'merchant') ? 'active' : '' ?>"><a
                                href="<?php echo e(url('/merchants')); ?>"><i
                                    class="fa fa-user"></i><?php echo app('translator')->get('message.dashboard.nav-menu.merchants'); ?></a></li>
                <?php endif; ?>
                <?php if(auth()->user()->type == 'merchant'): ?>                    
                 <li class="<?= isset($menu) && ($menu == 'my_store') ? 'active' : '' ?>">                        
                     <a href="<?php echo e(url('/mystore')); ?>" class="nav-link">                            
                     <i class="fa fa-spinner"></i> My Store                       
                     </a>                    
                </li> 
                <!--<li class="<?= isset($menu) && ($menu == 'shipping_cost') ? 'active' : '' ?>">                        -->
                <!--     <a href="<?php echo e(url('/shipping_cost')); ?>" class="nav-link">                            -->
                <!--     <i class="fa fa-spinner"></i> Shipping Cost                       -->
                <!--     </a>                    -->
                <!--</li>   -->
                
                 <?php endif; ?>
       <?php if(Common::has_permission(auth()->id(),'manage_transaction')): ?>
            <li class="<?= isset($menu) && ($menu == 'transactions') ? 'active' : '' ?>"><a href="<?php echo e(url('/transactions')); ?>"><i class="fa fa-file"></i><?php echo app('translator')->get('message.dashboard.nav-menu.transactions'); ?></a></li>
        <?php endif; ?>
       <?php if((empty($persona_kyc) && !empty($manual_kyc)) || (!empty($persona_kyc) && empty($manual_kyc)) || (!empty($persona_kyc) && !empty($manual_kyc))): ?>
        ...
        <!--<?php if(Common::has_permission(auth()->id(),'manage_deposit')): ?>-->
        <!--    <li>-->
        <!--        <a href="<?php echo e(url('/topup')); ?>"><i class="fa fa-mobile"></i><span>Mobile Topup</span></a>-->
        <!--    </li>-->
        <!--<?php endif; ?>-->
          <?php if(Common::has_permission(auth()->id(),'manage_deposit') && auth()->user()->role_id==2): ?>
            <li>
        ...
        <?php if(Common::has_permission(auth()->id(),'manage_transfer')): ?>
            <li class="<?= isset($menu) && ($menu == 'send_receive') ? 'active' : '' ?>"><a href="<?php echo e(url('/moneytransfer')); ?>"><i class="fa fa-share-square"></i><?php echo app('translator')->get('message.dashboard.nav-menu.send-req'); ?></a></li>
        ...
        <?php endif; ?>
    <?php endif; ?>
</ul><?php /**PATH /home/ticktappay/public_html/paymoney/resources/views/user_dashboard/layouts/common/header.blade.php ENDPATH**/ ?>