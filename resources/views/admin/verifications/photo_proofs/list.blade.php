...
@section('page_content')
    ...
    <div class="box box-default">
        <div class="box-body">
            <div class="row">
                <div class="col-md-8">
                    <div class="top-bar-title padding-bottom pull-left">Photo Verification</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="box">
        <div class="box-body">
            <form class="form-horizontal" action="{{ url('admin/photo-proofs') }}" method="GET">
                        ...
                        <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                @foreach($documents as $k=>$document)
                                    ...
                                    <tr>
                                        <td>{{++$k}}</td>
                                        <td>{{$user_id->carib_id}}</td>
                                        <td><a href="{{ url('admin/users/edit/' . $document->user_id)}}">{{$sender}}</a></td>
                                        <td>
                                        ...
                                        <td>{{ Carbon\Carbon::parse($document->updated_at)->format('d-M-Y h:i A') }}</td>
                                        <td>{{$updated_by}}</td>
                                        <td>{{$location}}</td>
                                        <td>{{$document->app_ver}}</td>
                                        <td>{{$document->device_manufacture.' | '.$document->device_name.' | '.$document->device_model}}</td>
                                        <td>{{$document->device_os.' '.$document->os_ver}}</td>
                                        <td>{{$document->ip_address}}</td>
                                        <td class=" dt-center">
                                            @if(Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_identity_verfication'))
                                                <a href="{{ url('admin/photo-proofs/edit/' . $document->id) }}" class="label label-primary"><i class="glyphicon glyphicon-edit"></i></a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
