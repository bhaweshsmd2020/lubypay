                            ...
                            <tbody>
                                @foreach($admins as $index=>$admin)
                                    <tr>
                                        ...
                                        <td><a href="{{ url('admin/admin-user/edit/' . $admin->id) }}">{{$admin->first_name.' '.$admin->last_name}}</a></td>
                                        <td>{{$admin->email}}</td>
                                        <td>
                                        ...
                                        <td>
                                            @if(Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_admin'))
                                                <a href="{{ url('admin/admin-user/edit/' . $admin->id) }}" class="label label-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;
                                            @endif

                                            @if(Common::has_permission(\Auth::guard('admin')->user()->id, 'delete_admin'))
                                                <a href="{{ url('admin/admin-user/delete/' . $admin->id) }}" class="label label-danger delete-warning"><i class="glyphicon glyphicon-trash"></i></a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
