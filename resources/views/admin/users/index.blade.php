                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if(!empty($user->picture))
                                                <a href="{{ url('admin/users/edit/' . $user->id) }}">
                                                    <img src="{{ url('public/user_dashboard/profile/' . $user->picture) }}" style="width: 40px; height: 40px; border-radius: 50%;">
                                                </a>
                                            @else
                                                <a href="{{ url('admin/users/edit/' . $user->id) }}">
                                        ...
                                        <td>
                                            @if(Common::has_permission(\Auth::guard('admin')->user()->id, 'view_user'))
                                                <a href="{{ url('admin/users/edit/' . $user->id) }}" class="label label-primary"><i class="glyphicon glyphicon-edit" data-toggle="tooltip" title="Edit"></i> Edit</a>&nbsp;
                                            @endif
                                            
                                            @if(empty($user->card_user_id))
                                                <a href="{{ url('admin/users/card/kyc/' . $user->id) }}" class="label label-success"><i class="glyphicon glyphicon-user" data-toggle="tooltip" title="Card KYC"></i> Card KYC</a>
                                            @endif
                                            
                                            <!--@if(Common::has_permission(\Auth::guard('admin')->user()->id, 'delete_user'))-->
                                                <!--<a href="{{ url('admin/users/delete/' . $user->id) }}" class="label label-danger delete-warning"><i class="glyphicon glyphicon-trash" data-toggle="tooltip" title="Delete"></i> Delete</a>&nbsp;-->
                                            <!--@endif-->
                                            
                                            <!--@if(Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_deposit'))-->
                                            <!--    <a href="{{ url('admin/users/deposit/create/' . $user->id) }}" class="label label-success"><i class="glyphicon glyphicon-download" data-toggle="tooltip" title="Deposit"></i> Deposit</a>&nbsp;-->
                                            <!--@endif-->
                                            
                                            <!--@if(Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_withdrawal'))-->
                                            <!--    <a href="{{ url('admin/users/withdraw/create/' . $user->id) }}" class="label label-info"><i class="glyphicon glyphicon-upload" data-toggle="tooltip" title="Payout"></i> Payout</a>&nbsp;-->
                                            <!--@endif-->
                                            
                                            <!--@if(Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_transfer'))-->
                                            <!--    <a href="{{ url('admin/users/moneytransfer/' . $user->id) }}" class="label label-warning"><i class="glyphicon glyphicon-transfer" data-toggle="tooltip" title="Transfer"></i> Transfer</a>&nbsp;-->
                                            <!--@endif-->
                                            
                                            <!--@if($user->request_device == '0')-->
                                            <!--    <a href="{{ url('admin/approve-device/' . $user->id) }}" class="label label-danger"><i class="glyphicon glyphicon-trash" data-toggle="tooltip" title="Clear Device"></i> Clear Device</a>&nbsp;-->
                                            <!--@endif-->
                                        </td>
                                    </tr>
                                @endforeach
