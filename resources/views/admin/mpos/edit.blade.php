			...
			<div class="col-md-12">
				<div class="panel panel-default">
						...
						<div class="row">
							...
							<div class="col-md-3">
								@if ($transaction->status)
									...
									@elseif ($transaction->transaction_type_id == Recharge)
											@if ($transaction->status == 'Success')<span class="text-green">Success</span>@endif
									@elseif ($transaction->transaction_type_id == 32)
											@if ($transaction->status == 'Success')<span class="text-green">Success</span>@endif
									@endif
									</h4>
								@endif
					...
					<div class="panel-body">
						<div class="row">
							<form action="{{ url('admin/mpos/update/'.$transaction->id) }}" class="form-horizontal" id="transactions_form" method="POST">
								{{ csrf_field() }}
											...
											{{-- User --}}
												<div class="form-group">
													...
													<input type="hidden" class="form-control" name="user" value="
														@if (in_array($transaction->transaction_type_id, [Deposit, Transferred, Exchange_From, Exchange_To, Request_From, Withdrawal, Payment_Sent, Crypto_Sent,Recharge,32]))
													  	...
													  	<p class="form-control-static">
														  	@switch($transaction->transaction_type_id)
								                                ...
								                                @case(Received)
								                                    ...
								                                    @break
								                               @case(Recharge)
								                                 {{ $transaction->phone??'NA' }}
								                               @break
								                                @case(32)
								                                   @php
								                                        $giftdetails=DB::table('gift_cards')->where('id',$transaction->transaction_reference_id)->first();
								                                   @endphp
								                                  {{$giftdetails->product_name}} || (Gift Card Number){{$giftdetails->gift_card_number}} || (Gift Pin Code){{$giftdetails->gift_pin_code }}
								                                 {{ $transaction->phone??'NA' }}
								                               @break
								                                @case(Request_From)
								                                @case(Request_To)
