	...
	<div class="container">
		<div class="row">
			<div class="col-md-7 col-xs-12 mb20 marginTopPlus">
				...
				<form action="{{url('merchant/shippingcostupdate')}}"  method="post" enctype="multipart/form-data" accept-charset="utf-8" id="shippingcost_add_form">
					<input type="hidden" value="{{csrf_token()}}" name="_token" id="token">
					
					<input type="hidden" value="{{$details['id']}}" name="id" id="id">
								...
								<?php $carrier = DB::table('carriers')->get();?>
								 <select class="form-control" name="carriers" id="carriers">
                                    ...
                                    @foreach ($carrier as $carriers)
                                        <option value="{{ $carriers->id }}" {{($carriers->id  ==$details['carrier_id']) ? 'selected' :''}} >{{ $carriers->name }}</option>
                                    @endforeach
                                </select>
								...
								<?php $countries = DB::table('countries')->get();?>
								 <select class="form-control" name="country" id="country">
                                    ...
                                    @foreach ($countries as $country)
                                        <option value="{{ $country->id }}" {{($country->id  ==$details['country']) ? 'selected' :''}}>{{ $country->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <label for="exampleInputPassword1">@lang('message.dashboard.send-request.common.currency')</label>
                                <select class="form-control" name="currency_id">
                                    @foreach($activeCurrencies as $result)
                                            <option value="{{ $result->id }}" {{ ($defaultWallet) && $defaultWallet->currency_id == $result->id ? 'selected="selected"' : '' }}>{{ $result->code }}</option>
                                    @endforeach
                                </select>
                            </div>
