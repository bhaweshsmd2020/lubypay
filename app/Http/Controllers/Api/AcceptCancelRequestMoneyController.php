...
class AcceptCancelRequestMoneyController extends Controller
{
    ...
    public function requestAcceptedConfirm(Request $request)
    {
         //dd(request()->all());
        ...
        $senderInfo          = User::where(['id' => $user_id])->first(['email','first_name']);
        $userInfo            = $this->helper->getEmailPhoneValidatedUserInfo($emailFilterValidate, $phoneRegex, trim($tr_email_or_phone));
    	...
    	// For Sender
        $sender_device = DB::table('devices')->where('user_id', $userInfo->id)->first();
        if(!empty($sender_device)){
            $sender_template = NotificationTemplate::where('temp_id', '8')->where('language_id', $sender_device->language)->first();
            $sender_subject = $sender_template->title;
            $sender_subheader = $sender_template->subheader;
            $sender_message = $sender_template->content;
            
            $send_msg = str_replace('{receiver}', $senderInfo->first_name, $sender_message);
            ...
            $type      = "acceptmoney";
            $this->helper->sendFirabasePush($sender_subject, $send_msg, $userInfo->id, $currency, $type);
            ...
            Noticeboard::create([
                ...
                'receiver' => $senderInfo->first_name,
            ]);
        }
        
    	// For Reciver
    	$receiver_device = DB::table('devices')->where('user_id', $user_id)->first();
    	if(!empty($receiver_device)){
        	$receiver_templates = NotificationTemplate::where('temp_id', '12')->where('language_id', $receiver_device->language)->first();
            ...
            $receiver_subhead = str_replace('{sender}', $userInfo->first_name, $receiver_subheader);
            ...
            $receive_msg = str_replace('{sender}', $userInfo->first_name, $receiver_message);
            ...
            $currency  = request('currency_id');
            $type      = "acceptmoney";
            $this->helper->sendFirabasePush($receiver_subject, $receive_msg, $user_id, $currency, $type);
    ...
    {
        $userdevice = DB::table('devices')->where('user_id', $RequestPayment->receiver->id)->first();
        if(!empty($userdevice)){
        ...
        $englishSenderLanginfo = EmailTemplate::where([
            'temp_id'     => 5,
    ...
    public function requestCancel(Request $request)
    {
        ...
        $feesLimit           = $this->helper->getFeesLimitObject([], Request_To, $currency_id, null, null, ['charge_percentage', 'charge_fixed']);
        $userInfo            = $this->helper->getEmailPhoneValidatedUserInfo($emailFilterValidate, $phoneRegex, trim($tr_email_or_phone));
        $senderInfo          = User::where(['id' => $user_id])->first(['first_name','email']);
        try
        {
            \DB::beginTransaction();
            $TransactionA         = Transaction::where('transaction_reference_id', $tr_ref_id)->where('user_id', $user_id)->first();
            ...
            $sender_device = DB::table('devices')->where('user_id', $userInfo->id)->first();
            if(!empty($sender_device)){
                $sender_template = NotificationTemplate::where('temp_id', '9')->where('language_id', $sender_device->language)->first();
                $sender_subject = $sender_template->title;
                $sender_subheader = $sender_template->subheader;
                $sender_message = $sender_template->content;
        	...
        	$receiver_device = DB::table('devices')->where('user_id', $user_id)->first();
        	if(!empty($receiver_device)){
        	    $receiver_templates = NotificationTemplate::where('temp_id', '13')->where('language_id', $receiver_device->language)->first();
                $receiver_subject = $receiver_templates->title;
                $receiver_subheader = $receiver_templates->subheader;
                $receiver_message = $receiver_templates->content;
    ...
    public function sendRequestCancelNotificationToAcceptorOrCreatorForApi($RequestPayment, $tr_email_or_phone, $user_id)
    {
        ...
        if (checkAppMailEnvironment())
        {
            ...
            elseif (!empty($RequestPayment->receiver_id) && $user_id == $RequestPayment->receiver_id)
            {
                ...
                $acceptor = User::where('id', $RequestPayment->user_id)->first();
                ...
                $twoStepVerification_msg = str_replace('{creator}', $user->first_name . ' ' . $user->last_name, $twoStepVerification->body);
                $twoStepVerification_msg = str_replace('{acceptor}', $acceptor->first_name . ' ' . $acceptor->last_name, $twoStepVerification_msg);
                $twoStepVerification_msg = str_replace('{amount}', $RequestPayment->amount, $twoStepVerification_msg);
                $twoStepVerification_msg = str_replace('{created_at}', $RequestPayment->local_tran_time, $twoStepVerification_msg);
                $twoStepVerification_msg = str_replace('{uuid}', $RequestPayment->uuid, $twoStepVerification_msg);
                $twoStepVerification_msg = str_replace('{currency}', $RequestPayment->currency->code, $twoStepVerification_msg);
                ...
                $twoStepVerification_sub = $twoStepVerification->subject;
                $twoStepVerification_msg = str_replace('{receiver}', $user->first_name . ' ' . $user->last_name, $twoStepVerification->body);
                $twoStepVerification_msg = str_replace('{sender}', $acceptor->first_name . ' ' . $acceptor->last_name, $twoStepVerification_msg);
                $twoStepVerification_msg = str_replace('{amount}', $RequestPayment->amount, $twoStepVerification_msg);
                $twoStepVerification_msg = str_replace('{created_at}', $RequestPayment->local_tran_time, $twoStepVerification_msg);
                $twoStepVerification_msg = str_replace('{uuid}', $RequestPayment->uuid, $twoStepVerification_msg);
                $twoStepVerification_msg = str_replace('{currency}', $RequestPayment->currency->code, $twoStepVerification_msg);
