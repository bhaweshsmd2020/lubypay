    ...
    public function index()
    {
        ...
        if (isset($_GET['btn']))
        {
            ...
            if (empty($_GET['from']))
            {
                $data['from'] = $from  = null;
                $data['to'] = $to    = null;
                $data['transfers'] = (new Transfer())->getTransfersList($from, $to, $status, $currency, $user)->orderBy('id', 'desc')->get();
            }
            else
            {
        ...
        Transfer::where('read_status', '0')->update(['read_status' => '1']);
        return view('admin.transfers.list', $data);
    }
