...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['todayregistered']    = User::where('type', 'user')->whereDate('created_at', $today)->count();
        $data['todaytransactions']    = Transaction::whereDate('created_at', $today)->whereNotIn('transaction_type_id', ['34', '35'])->sum('subtotal');
        $data['todaydeposit']    = Transaction::whereDate('created_at', $today)->where('transaction_type_id',1)->sum('subtotal');
        $data['todaypayout']    = Transaction::whereDate('created_at', $today)->where('transaction_type_id',2)->sum('subtotal');
        
        $data['totalMerchant']   = User::where('type', 'merchant')->count();
        $data['totalStore']   = Store::count();
        $data['mpos_todayregistered']    = User::where('type', 'merchant')->whereDate('created_at', $today)->count();
        $data['mpos_todaytransactions']    = Transaction::whereDate('created_at', $today)->whereIn('transaction_type_id', ['34', '35'])->sum('subtotal');
