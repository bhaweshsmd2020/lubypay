    ...
    public function index()
    {
        ...
        if (isset($_GET['btn']))
        {
            ...
            if (empty($_GET['from']))
            {
                $data['from'] = $from  = null;
                $data['to'] = $to    = null;
                $data['requests'] = (new RequestPayment())->getRequestPaymentsList($from, $to, $status, $currency, $user)->orderBy('id', 'desc')->get();
            }
            else
            {
        ...
        RequestPayment::where('read_status', '0')->update(['read_status' => '1']);
        return view('admin.RequestPayment.list', $data);
    }
    ...
    public function update(Request $request)
    {
       //  dd($request->all());
        ...
        if ($request->transaction_type == 'Request_To')
        {
            ...
            {
                 //die('Request_Created => Request: Blocked, DB Status: Pendinggggggggggggggggggg');
