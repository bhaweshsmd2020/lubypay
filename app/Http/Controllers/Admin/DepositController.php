...
class DepositController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['deposit_list']   = $deposit   = Deposit::orderBy('id', 'desc')->get();
        ...
        if (isset($_GET['btn']))
        {
            ...
            if (empty($_GET['from']))
            {
                $data['from'] = $from  = null;
                $data['to'] = $to    = null;
                $data['deposits'] = (new Deposit())->getDepositsList($from, $to, $status, $currency, $pm, $user)->orderBy('id', 'desc')->get();
            }
            else
            {
        ...
        Deposit::where('read_status', '0')->update(['read_status' => '1']);
