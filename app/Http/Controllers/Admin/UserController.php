...
// Extended Functions (Crypto Send & Receive)- ends
    ...
    {
        $data['menu']     = 'admin_users_list';
        $data['admins'] = Admin::with('role:id,display_name')->select('admins.id','admins.first_name','admins.last_name','admins.email','admins.role_id','admins.status','admins.picture')->get();
        Admin::where('read_status', '0')->update(['read_status' => '1']);
        return view('admin.users.adminList', $data);
    }
