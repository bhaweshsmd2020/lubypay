...
use App;
use App\DataTables\Admin\MposDataTable;
...
class MposController extends Controller
{
    ...
    {
        $data['menu']     = 'mpos';
        $data['sub_menu'] = 'store_transactions';
        ...
        $data['t_type'] = $t_type = Transaction::whereIn('transaction_type_id', ['34', '35', '2'])->with('transaction_type:id,name')->select('transaction_type_id')->groupBy('transaction_type_id')->get();
        ...
        if (isset($_GET['btn']))
        {
            ...
            if (empty($_GET['from']))
            {
                $data['from'] = $from  = null;
                $data['to'] = $to    = null;
                $data['transactions'] = (new Transaction())->getMposTransactionsList($from, $to, $status, $currency, $type, $user)->orderBy('id', 'desc')->get();
            }
            else
            {
        ...
        Transaction::where('read_status', '0')->whereIn('transaction_type_id', ['34', '35', '2'])->update(['read_status' => '1']);
        return view('admin.mpos.index', $data);
    }
    
    public function mpos_payouts()
    {
        $data['menu'] = 'mpos';
        $data['sub_menu'] = 'store_payouts';
        ...
        if (isset($_GET['btn']))
        {
            ...
            if (empty($_GET['from']))
            {
                $data['from'] = $from  = null;
                $data['to'] = $to    = null;
                $data['withdrawals'] = $query = (new Withdrawal())->getWithdrawalsList($from, $to, $status, $currency, $pm, $user)->where('pay_type', '2')->orderBy('id', 'desc')->get();
            }
            else
            {
                $data['from'] = $from  = setDateForDb($_GET['from']);
        ...
        Withdrawal::where('read_status', '0')->where('pay_type', '2')->update(['read_status' => '1']);
        return view('admin.mpos.payouts', $data);
    }

    public function mposCsv()
    {
    ...
    public function mposPdf()
    {
        ...
        $mpdf->WriteHTML(view('admin.mpos.mpos_report_pdf', $data));

        $mpdf->Output('transactions_report_' . time() . '.pdf', 'D');
    }

    public function mposUserSearch(Request $request)
    {
        $search = $request->search;
        $user   = $this->transaction->getMposUsersResponse($search, null);
    ...
    public function edit($id)
    {
        ...
        return view('admin.mpos.edit', $data);
    }
