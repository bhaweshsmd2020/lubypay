...
class PagesController extends Controller
{
    ...
    public function edit($page_id, $language_id)
    {
        $data['menu'] = 'settings';
        $data['sub_menu'] = 'general';
        $data['sub_sub_menu'] = 'pages';
        $data['page'] = $page = Pages::find($page_id);
        $data['language'] = $language = DB::table('all_languages')->where('id', $language_id)->first();
        $data['all_languages'] = $all_languages = DB::table('all_languages')->get();
    ...
    {
        $validation = Validator::make($request->all(), [
            'name'    => 'required|unique:pages,en,' . $request['id'],
            'content' => 'required',
        ]);
        ...
        if($request->language_id == '1'){
            $page->string     = trim($request->name);
            $page->string_content = $dom->saveHTML();
        }if($request->language_id == '2'){
            $page->en     = trim($request->name);
            $page->en_content = $dom->saveHTML();
        }if($request->language_id == '3'){
            $page->es     = trim($request->name);
            $page->es_content = $dom->saveHTML();
        }if($request->language_id == '4'){
            $page->fr     = trim($request->name);
            $page->fr_content = $dom->saveHTML();
        }if($request->language_id == '5'){
            $page->haiti     = trim($request->name);
            $page->haiti_content = $dom->saveHTML();
        }if($request->language_id == '6'){
            $page->pt     = trim($request->name);
            $page->pt_content = $dom->saveHTML();
        }
