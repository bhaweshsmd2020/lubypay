...
class TicketController extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($_GET['btn']))
        {
            ...
            if (empty($_GET['from']))
            {
                $data['from'] = $from  = null;
                $data['to'] = $to    = null;
                $data['tickets'] = (new Ticket())->getTicketsList($from, $to, $status, $user)->orderBy('id', 'desc')->get();
            }
            else
            {
        ...
        Ticket::where('read_status', '0')->update(['read_status' => '1']);
    ...
    public function store(Request $request)
    {
        ...
        $ticket->read_status = '0';
        $ticket->subject          = $request->subject;
        $ticket->message          = $dom->saveHTML();
        $ticket->code             = 'TIC-' . Str::random(6);
        ...
        if ($request->hasFile('file'))
        {
            ...
            if ($file_extn == 'docx' || $file_extn == 'rtf' || $file_extn == 'doc' || $file_extn == 'pdf' || $file_extn == 'png' || $file_extn == 'jpg' || $file_extn == 'jpeg' || $file_extn == 'gif' || $file_extn == 'bmp')
            {
        ...
        if (isset($ticket->user_id))
        {
            ...
            $type = "Message";
            $subheader = 'Ticket Created Successfully!';
            $date    = date("m-d-Y h:i");
            $message = $request->message;
            $this->helper->sendFirabasePush($subject,$subheader,$request->user_id, $currency, $type);
    ...
    public function reply($id)
    {
        ...
        $data['menu'] = 'ticket';
        $data['ticket'] = $ticket = Ticket::with('file')->find($id);
        $data['ticket']->user_id;
        Ticket::where('id',$id)->update(['read_status'=>0]);
        $data['ticket_status'] = $ticket_status = TicketStatus::get();
        TicketReply::where(['ticket_id' => $id])->update(['status'=>1]);
    ...
    public function change_ticket_status(Request $request)
    {
        ...
        {
            $update = Ticket::where(['id' => $request->ticket_id])->update(['ticket_status_id' => $request->status_id, 'read_status'=>'0']);
    ...
    public function adminTicketReply(Request $request)
    {
        ...
        $ticket_reply->message   = $dom->saveHTML();
        $ticket_reply->save();
    	...
    	$userdevice = DB::table('devices')->where('user_id', $request->user_id)->first();
    	if(!empty($userdevice)){
            $template = NotificationTemplate::where('temp_id', '22')->where('language_id', $userdevice->language)->first();
            $subject = $template->title;
            $subheader = $template->subheader;
            $tmessage = $template->content;
            
            $sub = str_replace('{ticket}', $ticket_reply->ticket->code, $subject);
            $subhead = str_replace('{ticket}', $ticket_reply->ticket->code, $subheader);
            $msg = str_replace('{ticket}', $ticket_reply->ticket->code, $tmessage);
            ...
            Noticeboard::create([
                ...
                'ticket' => $ticket_reply->ticket->code
            ]);
    	}
