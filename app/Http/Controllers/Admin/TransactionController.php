...
class TransactionController extends Controller
{
    ...
    {
        $data['menu'] = 'users';
        $data['sub_menu'] = 'all_transactions';
        ...
        $data['t_type'] = $t_type = Transaction::whereNotIn('transaction_type_id', ['34', '35'])->with('transaction_type:id,name')->select('transaction_type_id')->groupBy('transaction_type_id')->get();
        ...
        if (isset($_GET['btn']))
        {
            ...
            if (empty($_GET['from']))
            {
                $data['from'] = $from  = null;
                $data['to'] = $to    = null;
                $data['transactions'] = (new Transaction())->getTransactionsList($from, $to, $status, $currency, $type, $user)->orderBy('id', 'desc')->get();
            }
            else
            {
        ...
        Transaction::where('read_status', '0')->whereNotIn('transaction_type_id', ['34', '35'])->update(['read_status' => '1']);
