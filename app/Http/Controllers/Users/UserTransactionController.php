...
class UserTransactionController extends Controller
{
    ...
    public function index()
    {
        ...
        if($requested_for_withdrawal->count() > 0){
            ...
            $not_withdrawn = Transaction::where(['transaction_type_id' => '12', 'status' => 'Success', 'user_id' => Auth::user()->id])->whereNotIn('id', $ids)->get();
        }
        
        $data['sale_transactions'] = Transaction::where(['transaction_type_id' => '12', 'status' => 'Success', 'user_id' => Auth::user()->id])->get();
        ...
        if ($transaction->count() > 0)
        {
            switch ($transaction->transaction_type_id)
            {
                ...
                //     break;
                    ...
                    "<div class='clearfix'></div>" .
                    "<div class='left '>" . __('message.dashboard.crypto.send.confirm.network-fee') . "</div>" .
                    "<div class='right '>" . moneyFormat($transaction->currency->symbol, formatNumber($transaction->charge_fixed)) . "</div>" .
                    "<div class='clearfix'></div>" .
                    "<hr/>" .
                    "<div class='left '><strong>" . __('message.dashboard.left-table.total') . "</strong></div>" .
                    "<div class='right '><strong>" . moneyFormat($transaction->currency->symbol, formatNumber($transaction->total)) . "</strong></div>" .
