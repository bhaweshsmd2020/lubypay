...
class TicketController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // new code rajesh
         $englishResetTempInfo = EmailTemplate::where(['temp_id' => 11, 'lang' => 'en', 'type' => 'email'])->select('subject', 'body')->first();
         $ticketdetails = Ticket::with(['ticket_status:id,name','user:id,first_name,last_name,picture'])->find($ticket->id);
         $user  = User::where('id', Auth::user()->id)->first();
         $userFullName = $user->first_name . ' ' . $user->last_name;
         $useremail = $user->email;
         $ticketcode = $ticketdetails->code;
        $ticketInfo_sub_admin = $englishResetTempInfo->subject;
        $ticketInfo_msg_admin = 'Hi , <br><b>'.$userFullName.'</b> Ticket Created. ';
        $ticketInfo_msg_admin .= '<br><br>User Full Name : '.$userFullName;
        $ticketInfo_msg_admin .= '<br>User Email     : '.$useremail;
        $ticketInfo_msg_admin .= '<br>Ticket code      : '.$ticketcode;
        $ticketInfo_msg_admin .= '<br><br><br>Regards <br><b>'.getCompanyName().'</b>';
        $this->email->setupEmailConfig();
        $emailSetting = NotificationSetting::getSettings(['nt.alias' => 'request', 'notification_settings.recipient_type' => 'email', 'notification_settings.status' => 'Yes']);
         //$this->email->sendEmail($emailSetting[0]['recipient'], $ticketInfo_sub_admin, $ticketInfo_msg_admin);
         // new code rajesh
         // new code rajesh
        $userdevices               = DB::table('devices')->where(['user_id' => $user->id])->first();
        $notifyData   = array (
...
// 		if($this->helper->sendFCMPush($notifyData))
// 		{
// 		    $notifyData['user']= $user->id;
// 		   Noticeboard::create($notifyData); 
// 		}
        // new code rajesh
    ...
    public function reply_store(Request $request)
    {
        ...
        $this->helper->one_time_message('success', __('Ticket Reply Saved Successfully!'));
         ...
         // new code rajesh
         $englishResetTempInfo = EmailTemplate::where(['temp_id' => 12, 'lang' => 'en', 'type' => 'email'])->select('subject', 'body')->first();
         $ticketdetails = Ticket::with(['ticket_status:id,name','user:id,first_name,last_name,picture'])->find($request->ticket_id);
         $user  = User::where('id', Auth::user()->id)->first();
         $userFullName = $user->first_name . ' ' . $user->last_name;
         $useremail = $user->email;
         $ticketcode = $ticketdetails->code;
        $ticketInfo_sub_admin = $englishResetTempInfo->subject;
        $ticketInfo_msg_admin = 'Hi , <br><b>'.$userFullName.'</b>  initiated reply against ticket code #'.$ticketcode ;
        $ticketInfo_msg_admin .= '<br><br>User Full Name : '.$userFullName;
        $ticketInfo_msg_admin .= '<br>User Email     : '.$useremail;
        $ticketInfo_msg_admin .= '<br>Ticket code      : '.$ticketcode;
        $ticketInfo_msg_admin .= '<br>Mesage      : '.$request->description;
        $ticketInfo_msg_admin .= '<br><br><br>Regards <br><b>'.getCompanyName().'</b>';
        $this->email->setupEmailConfig();
        $emailSetting = NotificationSetting::getSettings(['nt.alias' => 'request', 'notification_settings.recipient_type' => 'email', 'notification_settings.status' => 'Yes']);
         $this->email->sendEmail($emailSetting[0]['recipient'], $ticketInfo_sub_admin, $ticketInfo_msg_admin);
