...
class ExchangeController extends Controller
{
    ...
    public function getCurrenciesExchangeRate(Request $request)
    {
        ...
        if (!empty($toWalletCurrency))
        {
            ...
            $getAmountMoneyFormat             = $destinationCurrencyRate * $request->total_amount;
            ...
            return response()->json([
                ...
                'getAmountMoneyFormatHtml' => moneyFormat($toWalletCurrency->code, formatNumber($getAmountMoneyFormat, $toWalletCurrency->id)), //just for show, not taken for further processing //fixed for pm v2.5
            ]);
        }
        else
        {
     ...
     public function exchangeOfCurrency(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            $request['finalAmount'] = $request->total_amount * $response->getData()->destinationCurrencyRate;
            ...
            $data['transInfo']['defaultAmnt']   = $request->total_amount;
            $data['transInfo']['totalAmount']   = ($request->total_amount) + $request->fee;
     ...
     public function exchangeOfCurrencyConfirm()
    {
        ...
        $data['transInfo']['finalAmount']   = $finalAmount;
