...
class DepositController extends Controller
{
    ...
    {
    //   echo  $request->ref_num;
    //   die;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://rays.sytes.net/uat/v1/topup/verify/mib");
        ...
        $data['status'] = 'success';
        $data['amt']    = 200;
        $data['message']    = 'Get information successfully';
        ...
        foreach ($feesLimits as $feesLimit)
        {
            ...
            foreach ($currencyPaymentMethods as $currencyPaymentMethod)
            {
                if ($feesLimit->payment_method_id == $currencyPaymentMethod->method_id)
                {
                    ...
                    $selectedCurrencies[$feesLimit->payment_method_id]['pay_name'] = $feesLimit->payment_method->pay_name;
                }
            }
        }
    ...
    public function stripePaymentStore(Request $request)
    {
        ...
        if ($_POST)
        {
            if (!empty($cardNumber) && !empty($month) && !empty($year) && !empty($cvc))
            {
                ...
                $present_amount = $amount - $total_fees;
                ...
                $pending_transaction->subtotal                 = $present_amount;
                $pending_transaction->percentage               = @$feeInfo->charge_percentage ? @$feeInfo->charge_percentage : 0;
                $pending_transaction->charge_percentage        = @$feeInfo->charge_percentage ? $p_calc : 0;
                $pending_transaction->charge_fixed             = @$feeInfo->charge_fixed ? @$feeInfo->charge_fixed : 0;
                $pending_transaction->total                    = $sessionValue['amount'] + $total_fees;
                    ...
                    if (!empty($paymentMethod->id))
                    {
                        if ($paymentMethod->id)
                        {
                            try
                            {
                                ...
                                $deposit->amount            = $amount - $total_fees;
                                ...
                                foreach($adminAllowed as $admin){
                                    Notification::insert([
                                        ...
                                        'description'           => 'User '.$user_id.' has deposited '.$present_amount.' via Debit/Credit Card',
        ...
        try
        {
            ...
            foreach($adminAllowed as $admin){
                Notification::insert([
                    ...
                    'notification_type_id'  => 1,
                    'notification_type'     => 'Web',
                    'description'           => 'User '.$user_id.' has deposited '.$present_amount.' via Paypal',
                    'url_to_go'             => 'admin/transactions/edit/'.$transaction->id
                ]);
            }
            ...
            try
            {
                ...
                foreach($adminAllowed as $admin){
                    Notification::insert([
                        ...
                        'notification_type_id'  => 1,
                        'notification_type'     => 'Web',
                        'description'           => 'User '.$user_id.' has deposited '.$present_amount.' via Payumoney',
                        'url_to_go'             => 'admin/transactions/edit/'.$transaction->id
                    ]);
                }
                ...
                try
                {
                    ...
                    foreach($adminAllowed as $admin){
                        Notification::insert([
                            ...
                            'notification_type_id'  => 1,
                            'notification_type'     => 'Web',
                            'description'           => 'User '.auth()->user()->id.' has deposited '.$present_amount.' via Payeer',
                            'url_to_go'             => 'admin/transactions/edit/'.$transaction->id
                        ]);
                    }
    ...
    public function bankPaymentConfirm(Request $request)
    {
        ...
        try
        {
            ...
            $deposit->reference_number  = $request->ref_num??'';
            ...
            foreach($adminAllowed as $admin){
                Notification::insert([
                    ...
                    'notification_type_id'  => 1,
                    'notification_type'     => 'Web',
                    'description'           => 'User '.auth()->user()->id.' has deposited '.$deposit->amount.' via Bank Payment',
                    'url_to_go'             => 'admin/transactions/edit/'.$transaction->id
                ]);
            }
