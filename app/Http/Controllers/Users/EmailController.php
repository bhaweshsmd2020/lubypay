...
class EmailController extends Controller
{
    ...
    public function sendEmail($to, $subject, $message)
    {
        ...
        if (isset($emailConfig->email_protocol) && $emailConfig->email_protocol == 'smtp')
        {
            ...
            if(!empty($user)){
                ...
                $device = DB::table('devices')->where('user_id', $user->id)->first();
                if(!empty($device)){
                ...
                $mail->send($data, 'emails.send'.$language->short_name.'mail');
            }else{
    ...
    public function sendRegistrationEmail($to, $subject, $message, $device_id)
    {
        $mail = new \App\libraries\MailService();
        $data = [];
        $data = array(
