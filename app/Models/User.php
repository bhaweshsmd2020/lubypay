...
class User extends Authenticatable
{
    ...
    {
        return $this->hasOne(Kycdatastore::class);
    }
    ...
    {
        return $this->hasMany(MerchantPackages::class,'user_id');
    }
    
    public function LoginLocation()
    {
        return $this->hasOne(LoginLocation::class,'user_id')->latest();
    }


    public function getPackageAttribute()
    {
        if(!$this->MerchantPackages->count()){
            $MerchantGroup = MerchantGroup::where('is_default', 'Yes')->first();
        ...
        return $this->MerchantPackages()->where('status','Approved')->first()->MerchantGroup->name;
    }
    ...
    public function getPackageIdAttribute()
    {
        ...
        return $this->MerchantPackages()->where('status','Approved')->first()->MerchantGroup->id;
    }
    ...
    {
        return $this->hasMany(MerchantDocument::class, 'user_id');
    }
    ...
    public function createNewUser($request, $intiatedBy)
    {
        ...
        {
            $countries = Country::where('name',$request->country)->first();
            $user->phone          = $request->phone;
            $user->defaultCountry = $request->country;
        ...
        $user->local_tran_time = $request->local_tran_time;
        $user->save();
        
        $carib_id = 'LP' . $user->id;
        
        if($user->id){
            $rs = User::where('id', $user->id)->update(['carib_id'=>$carib_id]);
        }

        if ($intiatedBy === 'merchant') {
            $MerchantGroup = MerchantGroup::where('is_default', 'Yes')->first();
            MerchantPackages::create([
                'user_id' => $user->id,
                'merchant_group_id' => $MerchantGroup->id,
     ...
     * param  [object] $userId
     */
    public function createUserDetail($request,$userId)
    {   
        $countries = Country::where('name',$request->country)->first();
        
        if($request->defaultCountry){
          $countries = Country::where('short_name',$request->defaultCountry)->first();
        }
    ...
    public function processMerchantEmailVerification($user)
    {
        //mail - temp -17
        $common                               = new Common();
        $englishUserVerificationEmailTempInfo = $common->getEmailOrSmsTemplate(52, 'email');
        $userVerificationEmailTempInfo        = $common->getEmailOrSmsTemplate(52, 'email', getDefaultLanguage());
        if (!empty($userVerificationEmailTempInfo->subject) && !empty($userVerificationEmailTempInfo->body))
        {
            // subject
            $userVerificationEmailTempInfo_sub = $userVerificationEmailTempInfo->subject;
