...
class RequestPayment extends Model
{
    ...
    public function getRequestPaymentsUserName($user)
    {
        ...
        ->with(['user:id,first_name,last_name,email','receiver:id,first_name,last_name,email'])
    ...
    public function getRequestPaymentsUsersResponse($search)
    {
        ...
        ->map(function ($requestPaymentA)
        {
            ...
            $arr['email']      = $requestPaymentA->user->email;
            return $arr;
        });
        //
        ...
        ->map(function ($requestPaymentB)
        {
            ...
            $arr['email']      = $requestPaymentB->receiver->email;
            return $arr;
        });
    ...
    public function createRequestPayment($arr)
    {
        ...
        $requestPayment->sender_email = $arr['senderEmail'];
        $requestPayment->local_tran_time = $arr['local_tran_time'];
        $requestPayment->ip_address = $arr['ip_address'];
        $requestPayment->save();
    ...
    public function createRequestFromTransaction($arr)
    {
        ...
        $transaction->local_tran_time = $arr['local_tran_time'];
        $transaction->ip_address = $arr['ip_address'];
        $transaction->save();
        ...
        $rs = TransDeviceInfo::create([
            ...
            'device_manufacture' => $arr['device_manufacture'], 
    ...
    public function receivemailToRegisteredRequestAcceptor($requestPayment, $arr)
    {
        ...
        if (checkAppMailEnvironment())
        {
            ...
            $req_create_msg = str_replace('{created_at}', $arr['local_tran_time'], $req_create_msg);
            ...
            try {
                $this->emailObject->sendEmail($arr['receierEmail'], $req_create_sub, $req_create_msg);
                return [
                    'ex' => null,
                ];
            }
    ...
    public function sendmailToRegisteredRequestAcceptor($requestPayment, $arr)
    {
        // dd($requestPayment, $arr);
        /**
         * Mail when request created
        ...
        if (checkAppMailEnvironment())
        {
            $englishSenderLanginfo = $this->helper->getEmailOrSmsTemplate(48, 'email');
            $req_create_temp       = $this->helper->getEmailOrSmsTemplate(48, 'email', getDefaultLanguage());

            if (!empty($req_create_temp->subject) && !empty($req_create_temp->body))
            {
                $req_create_sub = $req_create_temp->subject;
                $req_create_msg = str_replace('{sender_id}', $arr['senderName'], $req_create_temp->body);
            }
            else
            {
                $req_create_sub = $englishSenderLanginfo->subject;
                $req_create_msg = str_replace('{sender_id}', $arr['senderName'], $englishSenderLanginfo->body);
            }
            $req_create_msg = str_replace('{receiver_id}', $arr['receiverName'], $req_create_msg);
            $req_create_msg = str_replace('{amount}', moneyFormat($requestPayment->currency->symbol, formatNumber($arr['amount'])), $req_create_msg);
            $req_create_msg = str_replace('{uuid}', $arr['uuid'], $req_create_msg);
            $req_create_msg = str_replace('{created_at}', $arr['local_tran_time'], $req_create_msg);
            $req_create_msg = str_replace('{note}', $arr['note'], $req_create_msg);
            $req_create_msg = str_replace('{soft_name}', getCompanyName(), $req_create_msg);
            try {
                $this->emailObject->sendEmail($arr['senderEmail'], $req_create_sub, $req_create_msg);
                return [
                    'ex' => null,
                ];
            }
    ...
    public function receivemailToUnRegisteredRequestAcceptor($requestPayment, $arr)
    {
        /**
    ...
    public function sendmailToUnRegisteredRequestAcceptor($requestPayment, $arr)
    {
        /**
         * Mail to unregistered user when request created
         */
        if (checkAppMailEnvironment())
        {
            $email_explode                 = explode("@", trim($arr['senderEmail']));
            $unregisteredUserNameFromEmail = $email_explode[0];
            $profileName                   = getCompanyName();
            $subject                       = 'Notice of Request Send!';
            $message                       = 'Hi ' . $unregisteredUserNameFromEmail . ',<br><br>';
            $message .= 'You have send ' . moneyFormat($requestPayment->currency->symbol, formatNumber($arr['amount'])) . ' payment request to ' . $arr['receiver'] . '.<br><br>';
            $message .= 'Regards, <br>';
            $message .= $profileName;
    ...
    {
        $data = [];
        if (isset($arr['senderInfo']))
        {
            $mailToRegistUser = self::sendmailToRegisteredRequestAcceptor($requestPayment, $arr);
        ...
        {
            $mailToUnRegistUser = self::sendmailToUnRegisteredRequestAcceptor($requestPayment, $arr);
    ...
    public function receiveReqCreateEmailINotifnIfUserExists($requestPayment, $arr)
    {
        $data = [];
        if (isset($arr['userInfo']))
        {
            $mailToRegistUser = self::receivemailToRegisteredRequestAcceptor($requestPayment, $arr);
            if (!empty($mailToRegistUser['ex']))
            {
                $data = [
                    'exFrom' => 'mailToRegistUser',
        ...
        {
            $mailToUnRegistUser = self::receivemailToUnRegisteredRequestAcceptor($requestPayment, $arr);
            if (!empty($mailToUnRegistUser['ex']))
            {
                $data = [
                    'exFrom' => 'mailToUnRegistUser',
    ...
    public function processRequestCreateConfirmation($arr = [], $clearSessionFrom)
    {
        ...
        try
        {
            ...
            $receiver_transaction = self::createRequestToTransaction($arr);
            ...
            $response['transactionOrReqPaymentId'] = ($clearSessionFrom == 'web') ? $requestFromTransactionId : $requestPayment->id;
            $response['sender_transaction'] = $requestFromTransactionId;
            $response['receiver_transaction'] = $receiver_transaction;

            // Mail & SMS to registered request receiver
            $data = self::receiveReqCreateEmailINotifnIfUserExists($requestPayment, $arr);
            if (!empty($data))
            {
            ...
            // Mail & SMS to registered request sender
    ...
    {
        $requestPayment                = $this->with(['user:id,first_name,last_name,email,phone,carrierCode,email', 'receiver:id,first_name,last_name,email', 'currency:id,symbol,code'])->find($arr['requestPaymentId']);
    ...
    public function updateRequestAcceptorWallet($arr)
    {
        ...
        $requestAcceptorWallet->balance = $requestAcceptorWallet->balance - $arr['total'];
    ...
    public function onlyEmailToRequestCreatorOnRequestAccept($arr, $requestPayment)
    {
        if (checkAppMailEnvironment())
        {
            ...
            $rp_msg = str_replace('{created_at}', $arr['local_tran_time'], $rp_msg);
    ...
    public function onlyEmailToRequestAcceptorOnRequestAccept($arr, $requestPayment)
    {
        if (checkAppMailEnvironment())
        {
            //if other language's subject and body not set, get en sub and body for mail
            $englishSenderLanginfo = $this->helper->getEmailOrSmsTemplate(49, 'email');
            $rp_accept_temp        = $this->helper->getEmailOrSmsTemplate(49, 'email', getDefaultLanguage());
            if (!empty($rp_accept_temp->subject) && !empty($rp_accept_temp->body))
            {
                $rp_acc_sub = $rp_accept_temp->subject;
                $rp_msg     = str_replace('{sender}', $requestPayment->receiver->first_name . ' ' . $requestPayment->receiver->last_name, $rp_accept_temp->body);
            }
            else
            {
                $rp_acc_sub = $englishSenderLanginfo->subject;
                $rp_msg     = str_replace('{sender}', $requestPayment->receiver->first_name . ' ' . $requestPayment->receiver->last_name, $englishSenderLanginfo->body);
            }
            $rp_msg = str_replace('{uuid}', $requestPayment->uuid, $rp_msg);
            $rp_msg = str_replace('{receiver}', $requestPayment->user->first_name . ' ' . $requestPayment->user->last_name, $rp_msg);
            $rp_msg = str_replace('{created_at}', $arr['local_tran_time'], $rp_msg);
            $rp_msg = str_replace('{amount}', moneyFormat($requestPayment->currency->symbol, formatNumber($requestPayment->amount)), $rp_msg);
            $rp_msg = str_replace('{accept_amount}', moneyFormat($requestPayment->currency->symbol, formatNumber($requestPayment->accept_amount)), $rp_msg);
            $rp_msg = str_replace('{currency}', $requestPayment->currency->code, $rp_msg);
            ...
            try {
                $this->emailObject->sendEmail($requestPayment->receiver->email, $rp_acc_sub, $rp_msg);
                return [
                    'ex' => null,
                ];
            }
    ...
    public function processRequestAcceptConfirmation($arr = [], $clearSessionFrom)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                $receiver_transaction = self::udpateRequestCreatorTransaction($arr, $requestPayment);
                ...
                $response['sender_transaction'] = $reqAcceptTransactionId;
                $response['receiver_transaction'] = $receiver_transaction;
                ...
                // Mail when request accepted
                $mailToReqCrt = $this->onlyEmailToRequestAcceptorOnRequestAccept($arr, $requestPayment);
                if (!empty($mailToReqCrt['ex']))
                {
                    $response['ex']['source']  = 'mailToReqCrt';
                    $response['ex']['message'] = $mailToReqCrt['ex']->getMessage();
