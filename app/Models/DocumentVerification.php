...
class DocumentVerification extends Model
{
    ...
    public function getPhotoVerificationsList($from, $to, $status)
    {
        $conditions = [];
        ...
        if (!empty($date_range))
        {
            ...
            'user' => function ($query)
            {
                $query->select('id', 'first_name', 'last_name','carib_id');
            }])
        ...
        else
        {
            ...
            {
                $query->select('id', 'first_name', 'last_name','carib_id');
            }])->where(['verification_type' => 'photo'])->where($conditions)->select('document_verifications.*');
        }
