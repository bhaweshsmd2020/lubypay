...
class Withdrawal extends Model
{
    ...
    protected $fillable = ['user_id', 'currency_id', 'payment_method_id', 'uuid', 'charge_percentage', 'charge_fixed', 'subtotal', 'amount', 'payment_method_info', 'status', 'pay_type'];
    ...
    public function getWithdrawalsList($from, $to, $status, $currency, $pm, $user)
    {
        ...
        $withdrawals->where('withdrawals.pay_type', '2');
        //
        return $withdrawals;
    }
    ...
    public function createWithdrawal($arr)
    {
        ...
        $withdrawal->local_tran_time     = $arr['local_tran_time'];
        $withdrawal->ip_address          = $arr['ip_address'];
        $withdrawal->pay_type            = $arr['pay_type'];
        $withdrawal->save();
    ...
    public function createWithdrawalTransaction($arr)
    {
        ...
        $transaction->pay_type                 = $arr['pay_type'];
        $transaction->save();
