...
class CurrencyExchange extends Model
{
    ...
    public function getExchangesList($from, $to, $status, $currency, $user)
    {
        ...
        if (!empty($date_range))
        {
            $data = DB::select("
                ...
                WHERE $condtionWithDate ORDER BY id DESC
            ");
        }
        else
        {
            $data = DB::select("SELECT
                ...
                LEFT JOIN currencies as tc ON tc.id = to_currecny_id
                LEFT JOIN users ON users.id = detail.user_id
                WHERE $condtionsWithoutDate ORDER BY id DESC
            ");
        }
        return $data;
    }
    ...
    public function createCurrencyExchange($arr, $toWallet)
    {
        ...
        $currencyExchange->local_tran_time = $arr['local_tran_time'];
        $currencyExchange->ip_address    = $arr['ip_address'];
    ...
    public function createExchangeFromTransaction($arr, $currencyExchangeId)
    {
        ...
        $transaction->subtotal                 = $arr['amount'] - $arr['formattedChargePercentage'] - $arr['charge_fixed'];
    ...
    {
        $arr['fromWallet']->balance = ($arr['fromWallet']->balance - $arr['amount']);
        $arr['fromWallet']->save();
    }
    ...
    public function processExchangeMoneyConfirmation($arr = [], $clearSessionFrom)
    {
        ...
        try
        {
            ...
            $checkWalletBalance = $this->helper->checkWalletBalanceAgainstAmount($arr['amount'], $arr['fromWallet']->currency_id, $arr['user_id']);
            ...
            else
            {
                ...
                $sender_transaction = self::createExchangeFromTransaction($arr, $currencyExchange->id);
                ...
                $response['sender_transaction'] = $sender_transaction;
