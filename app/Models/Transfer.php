...
class Transfer extends Model
{
    ...
    public function createTransfer($arr)
    {
        ...
        $transfer->local_tran_time   = $arr['local_tran_time'];
        $transfer->ip_address        = $arr['ip_address'];
        $transfer->save();
    ...
    public function mailToSender($transfer, $arr)
    {
        if (checkAppMailEnvironment())
        {
            ...
            $sender_msg = str_replace('{created_at}', $arr['local_tran_time'], $sender_msg);
    ...
    public function mailToReceiver($transfer, $arr)
    {
        if (checkAppMailEnvironment())
        {
            ...
            if (isset($arr['userInfo']))
            {
                ...
                $receiver_msg = str_replace('{created_at}', $arr['local_tran_time'], $receiver_msg);
    ...
    public function processSendMoneyConfirmation($arr = [], $clearSessionFrom)
    {
        try
        {
            ...
            else
            {
                ...
                $receiver_transaction = self::createReceivedTransaction($arr);
                ...
                $response['transactionOrTransferId'] = ($clearSessionFrom == 'web') ? $transferredTransactionId : $transfer->id;
                $response['sender_transaction'] = $transferredTransactionId;
