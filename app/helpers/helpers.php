...
function sendSMSwithTwilio($twilioCredentials, $to, $message, $region)
{
    ...
    if($region == 'us-region'){
        ...
        $twilioNumber = $twilioCredentials['europe_twilio_phone_number'];
    }
...
function sendSMS($to, $message, $region)
{
    ...
    if (!empty($smsConfig)) {
        ...
        if (count($smsCredentials) > 0) {
            ...
            elseif ($smsConfig->type == 'twilio') {
                sendSMSwithTwilio($smsCredentials, $to, $message, $region);
            }
        }
    }
}
...
if (! function_exists('getPaymoneySettings')) {
    ...
    function getPaymoneySettings($type = null)
    {
        ...
        $array = [
            ...
            'payment_methods' => [
                'web' => [
                    'all' => [TickTapPay, Stripe, Paypal, PayUmoney, Bank, Coinpayments, Payeer, Crypto, Coinbase],
                    'deposit' => [TickTapPay, Stripe, Paypal, PayUmoney, Bank, Coinpayments, Payeer, Coinbase],
                    'withdrawal' => [Paypal, Bank, Crypto],
                    'fiat' => [
                        'deposit' => [TickTapPay, Stripe, Paypal, PayUmoney, Bank, Coinpayments, Payeer, Coinbase],
                        'withdrawal' => [TickTapPay, Paypal, Bank],
                    ],
                    'crypto' => [
                        'deposit' => [TickTapPay, Coinpayments, Coinbase],
                        'withdrawal' => [TickTapPay, Crypto],
                    ]
                ],
                'mobile' => [
...
if (!function_exists('getTransactionPaymentMethod')) {
    ...
    {
        return ('TickTapPay' == $methodName) ? settings('name') : $methodName;
    }
}
...
if (! function_exists('getPaymentDetails')) {
    ...
    function getPaymentDetails($payout)
    {
        ...
        } elseif ($payout->payment_method->name == "TickTapPay") {
...
function getMenuContent($position)
{
    $data = Pages::where('position', 'like', "%$position%")->where('status', 1)->get(['url', 'en']);
    return $data;
}
